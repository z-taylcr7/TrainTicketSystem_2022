...
namespace Geneva{
    ...
    template<int MM = 50, int LL = 50, int CACHESIZE = 250>
    class BPlusTree {
        ///Announcement:
        ///go left: <=key
        ///go right: >key
        ...
        public:
            ...
            void borrowLeft(BPlusTree *Tree, innerNode &left_bro, innerNode &father, int index) {
                ...
                father.nodeKey[index] = left_bro.nodeKey[left_bro.sum - 1];
                left_bro.sum--;
                sum++;
                Tree->memoInner->update(father, father.offset);
    ...
    private:
        ...
        eraseReturn BPErase(int cur, const Key &key) {
            innerNode curNode(memoInner->read(cur));
            int index = Geneva::upper_bound(curNode.nodeKey, curNode.nodeKey + curNode.sum, key);
            if (curNode.childIsLeaf) {
                leafNode toErase{memoLeaf->read(curNode.Pointer[index])};
                eraseReturn er;
                er.completed = toErase.deleteElement(this, key);
                er.remakeParent = toErase.resize(this, curNode, index);
                return er;
            } else {
                eraseReturn er{BPErase(curNode.Pointer[index], key)};
        ...
        std::pair<long long*,int> BPFind(int cur, const String &key) {
            //找一数列可行的
            long long* que= nullptr;int cnt=0;
            ...
            if (curNode.childIsLeaf) {
                ...
                if (pos == target.sum){
                    ...
                    if(pos==target.sum)return std::pair<long long*, int> {que, 0};}
                if (target.leafKey[pos].first == key) {
                    que=new long long[10000];
                    while(target.leafKey[pos].first == key){
                        que[cnt++]=target.leafKey[pos].second;
                        pos++;
                        if(pos==target.sum){
                            if(target.rightBro>=0){
                    ...
                    return std::pair<long long*, int> {que, cnt};
                }
                else return std::pair<long long*, int> {que, 0};
            } else {
                return BPFind(curNode.Pointer[index], key);
            }
        };
    ...
    public:
        ...
        bool erase(const Key &key) {
            ...
            if (rootNode.childIsLeaf) {
                ...
                eraseReturn er(BPErase(basicInfo.root,key));
        ...
        std::pair<long long* ,int> find(const String&key){
            long long* que=new long long[10000];int cnt=0;
            Key defaultKey= std::make_pair(key,0);
            if (basicInfo.size == 0 || basicInfo.root == -1)return std::pair<long long*,int> {nullptr, 0};
            int index = Geneva::upper_bound(rootNode.nodeKey, rootNode.nodeKey + rootNode.sum, defaultKey);
            if (rootNode.childIsLeaf) {
                leafNode target {memoLeaf->read(rootNode.Pointer[index])};
                int pos = Geneva::lower_bound(target.leafKey, target.leafKey + target.sum, defaultKey);
                if (pos == target.sum)return std::pair<long long*, int> {nullptr, 0};
                if (target.leafKey[pos].first == key) {
                ...
                else return std::pair<long long*, int> {nullptr, 0};
            }
            else {
                return BPFind(rootNode.Pointer[index], key);
            }
        }
